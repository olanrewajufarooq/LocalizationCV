...
class Homography:
    ...
    def _compute_homography_without_cv(self, input_points, output_points):
        ...
        # Solving without using normalization
        ...
        # Normalization
        row_means = A.mean(axis=0)
        C = np.diag((1/row_means))
        A_aug = A @ C
        
        U, s, Vt = np.linalg.svd(A_aug.T @ A_aug)
        min_eig_value_index = np.argmin(s)
        
        H_array_aug = U[:,min_eig_value_index]
        H_array = (C @ H_array_aug).reshape(-1,1)
    ...
    def _visualize_homographies(self, map_image_path, video_path, homography_matrices, frame_ids, visualization_delay=0):
        ...
        for frame_id in frame_ids:
            ...
            # output_image[:transformed_image.shape[0], map_image.shape[1]:] = transformed_image
             ...
             # Set opacity level (between 0 and 1)
            opacity_level = 0.5  # 50% opacity

             # Blend the transformed image with the map image
            blended_image = cv2.addWeighted(map_image, 1 - opacity_level, transformed_image, opacity_level, 0)
            # Show the output image
            cv2.imshow(f"Homography - Frame {frame_id}", blended_image)
    ...
    def compute_homography(self, pts_in_ref, pts_in_frame, frame_ids, corr_ref_ids, parserObject, map_image_path=None, video_path=None, visualization_delay=0):
        ...
        if self.visualize:
            ...
            self._visualize_homographies(map_image_path, video_path, H_output_allframe_to_map, frame_ids, visualization_delay=visualization_delay)
...
class ConfigParser:
    ...
    def save_features(self, features, frame_ids):
        
        if self.verbose:
            print(f"Saving features as to {self.config_dict['keypoints_out_path']} ...", end=" ")
            
        # Saving as MATLAB file
        savemat( f"{self.config_dict['keypoints_out_path']}", {"features": features, "frame_ids": frame_ids})
    ...
    def load_features(self):
        ...
        if self.verbose:
            print(f"Loading features from {self.config_dict['keypoints_out_path']}...", end=" ")
        
        # Loading from MATLAB file
        load_data = loadmat( self.config_dict['keypoints_out_path'] )
        features = load_data["features"].flatten()
        frame_ids = load_data["frame_ids"][0].astype(int).tolist()
...
# The code below runs if the src_file is called directly.
